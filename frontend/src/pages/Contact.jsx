    ...
    <div className="py-16 px-6 md:px-12 transition-colors duration-300">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-6xl mx-auto mb-16">
        {contactDetails.map((item, index) => (
          <div
            key={index}
            className="bg-white dark:bg-gray-900 p-6 text-center rounded-xl shadow-md border border-gray-200 dark:border-gray-700"
          >
            {item.icon}
            <h4 className="text-lg font-semibold text-gray-700 dark:text-white mb-1">{item.title}</h4>
            <p className="text-sm text-gray-500 dark:text-gray-300">{item.info}</p>
          </div>
        ))}
      </section>
      ...
      <section className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-700">
          <h3 className="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center">Send a Message</h3>
          <form className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Your Name</label>
              <input
                type="text"
                className="w-full p-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500"
            ...
            <div className="text-center mt-4">
              <button
                type="submit"
                className="bg-cyan-600 dark:bg-cyan-700 text-white px-6 py-3 rounded-md hover:bg-cyan-700 dark:hover:bg-cyan-800 transition"
              >
                Send Message
              </button>
            </div>
